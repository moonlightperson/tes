	...
	window.onload = function () {
			...
			let [header, list] = createLoadingList(`https://MoonLightPerson.github.io/BetterDiscordAddons/${type}/README.md`, {
				...
				getChildren: response => response.split("- [").splice(1).map(n => {
					...
					return name && (!createAddonList.search[type] || [name, description].filter(n => n).some(s => s.toLowerCase().indexOf(createAddonList.search[type].toLowerCase()) > -1)) && createAddonCard({
						...
						resourceUrl: `https://MoonLightPerson.github.io/BetterDiscordAddons/${type}/${fileName}/_res`,
			...
			return createLoadingList("https://MoonLightPerson.github.io/BetterDiscordAddons/Library/README.md", {
				...
				getChildren: response => {
					...
					return createAddonCard({
						...
						resourceUrl: "https://MoonLightPerson.github.io/BetterDiscordAddons/Library/_res",
		...
		const sections = [
			...
			{section: "Invites", renderSection: _ => [
				createInvite({invite: "Jx3TjNS", name: "DevilBro's Haus", details: "Support Server for all my Stuff", icon: "https://MoonLightPerson.github.io/_res/imgs/db_logo.webp"}),
				createElement(`<div class="divider"></div>`),
				createInvite({invite: "0Tmfo5ZbORCRqbAd", name: "BetterDiscord", details: "First Support Server for BD", icon: "https://MoonLightPerson.github.io/_res/imgs/bd1_logo.webp"}),
				createInvite({invite: "sbA3xCJ", name: "BetterDiscord2", details: "Second Support Server for BD", icon: "https://MoonLightPerson.github.io/_res/imgs/bd2_logo.webp"})
			]}
		];
		let items = [];
